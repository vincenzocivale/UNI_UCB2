...
# Per il training baseline, non applichiamo pruning, quindi il keep_ratio è 1.0
TRAINING_KEEP_RATIO = 1.0
...
# Anche se usiamo la stessa classe, con keep_ratio=1.0 non ci sarà pruning
...
args = TrainingArguments(
        output_dir="./results/baseline", # Salvataggio in una cartella dedicata
        run_name=f"ViT-Baseline-Training-keep_ratio-{TRAINING_KEEP_RATIO}",
...
# %% [markdown]
# ## Benchmark di Velocità Post-Training (Modello Baseline)
# 
# Dopo aver completato l'addestramento, la cella seguente eseguirà un benchmark per misurare la velocità di inferenza del modello baseline (non potato). Questo test:
# 1. Usa il modello appena addestrato.
# 2. Misura il tempo medio di inferenza usando il metodo `forward` standard.
# 3. Logga questo risultato su W&B per confronto.

# %%
# --- Benchmark di Velocità di Inferenza Post-Training (Baseline) ---
print("***** Inizio Benchmark di Velocità Post-Training (Baseline Model) *****")
...
# Per il modello baseline, misuriamo la velocità del forward pass standard
# senza alcun pruning.

# 1. Crea dati fittizi per il benchmark
batch_size = args.eval_batch_size
dummy_input = torch.randn(batch_size, 3, IMG_SIZE, IMG_SIZE, device=DEVICE)

# 2. Esegui il benchmark (con warmup)
print(f"Esecuzione di {WARMUP_RUNS} warmup runs...")
with torch.no_grad():
    for _ in range(WARMUP_RUNS):
        _ = trained_model(dummy_input)  # Usa il forward pass standard
if DEVICE.type == 'cuda':
...
print(f"Tempo medio di inferenza (Baseline): {avg_time_ms:.3f} ms")
print("---------------------------")

# 3. Logga il risultato su un nuovo progetto/run W&B
print("Logging della metrica di velocità su Weights & Biases...")
try:
    # Usiamo lo stesso progetto per un confronto facile
    wandb.init(
        project="vit-ucb-pruning-final",
        name=f"{args.run_name}-inference-benchmark",
        config={
            "inference_keep_ratio": 1.0,  # 1.0 per il baseline
            "benchmark_runs": BENCHMARK_RUNS,
            "training_run_name": args.run_name
        }
    )
    # Logga con un nome di metrica distinto per il baseline
    wandb.log({"baseline_inference_avg_time_ms": avg_time_ms})
    wandb.finish()
